...
uint8_t bme280::check(void) {		// check is it conected bme280,
...
bool bme280::is_meas(void)	{	// returns TRUE while bme280 is measuring								
	return (bool)((bme280::_f_read_reg(0xF3) & 0x08) >> 3);  	 // read status register & mask bit "measuring"
}

void bme280::do1meas(void) {    // operating mode FORCED_MODE do 1 measuring
	bme280::_f_write_reg( 0xF4, ((_reg_0xF4 & 0xFC) | 0x02) );   
}

struct_tph bme280::read_TPH(void) {
