...
bool bme280::f_reset(void) {     //  software reset of bme280
	if (bme280::f_write_reg(0x0E, 0xB6)) return true;
    else return false;
}

uint8_t bme280::f_read_reg(uint8_t address) {    // read 1 byte bme280 register
	Wire.beginTransmission(_i2c_address);
	Wire.write(address);
...
bool bme280::f_write_reg(uint8_t address , uint8_t data) {   //  write 1 byte to bme280 register
	Wire.beginTransmission(_i2c_address);
	Wire.write(address);
	Wire.write(data);
...
//	Wire.begin(); // start I2C interface
	...
	bme280::f_write_reg(0xF2, _reg_0xF2);   
	bme280::f_write_reg(0xF4, _reg_0xF4);   
	bme280::f_write_reg(0xF5, _reg_0xF5);
...
bool bme280::f_bme_is_meas(void)	{	// returns TRUE while bme280 is measuring								
	return (bool)((bme280::f_read_reg(0xF3) & 0x08) >> 3);  	 // read status register & mask bit "measuring"
}

void bme280::f_do_1_meas(void) {        // operating mode FORCED_MODE do 1 measuring
	bme280::f_write_reg( 0xF4, ((_reg_0xF4 & 0xFC) | 0x02) );   
}
